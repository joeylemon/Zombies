...
public class EffectUtils {
	...
	private static final World world = Utils.getWorld();
	...
	/**
	 * Play an empliy clip sound at the location
	...
	/**
	 * Play a break effect for a block
	 * @param block - The block
	 */
	public static final void playBreakEffect(Block block){
		for(Player p : Bukkit.getOnlinePlayers()){
			p.playEffect(block.getLocation(), Effect.STEP_SOUND, block.getType());
		}
	}
	...
	public static final void playBloodEffect(LivingEntity entity, boolean eye){
		for(Entity e : entity.getNearbyEntities(20, 20, 20)){
			if(e instanceof Player){
				Player p = (Player) e;
				if(eye == true){
					p.playEffect(entity.getEyeLocation(), Effect.STEP_SOUND, 152);
				}else{
